...
// @desc    Seed 25 premium events into the database
// @route   GET /api/seed
// @access  Public (one-time use, remove after seeding)
  ...
  try {
    ...
    if (!user) {
      return res.status(400).json({
        success: false,
        message: "No users found in the database. Please register at least one account first, then try again.",
      });
    }
    ...
    // Insert all seed events
    const eventsWithUser = EVENTS.map((e) => ({ ...e, createdBy: user._id }));
    const inserted = await Event.insertMany(eventsWithUser);

    res.status(200).json({
      success: true,
      message: `Seeded ${inserted.length} events! (Deleted ${deleted.deletedCount} old events)`,
      summary: {
        total: inserted.length,
        approved: inserted.filter((e) => e.approval_status === "approved").length,
        pending: inserted.filter((e) => e.approval_status === "pending").length,
        upcoming: inserted.filter((e) => e.status === "upcoming").length,
        completed: inserted.filter((e) => e.status === "completed").length,
        usingUser: user.email,
      },
    });
  } catch (error) {
